...
val prog = CompoundStm (AssignStm ("a", OpExp (NumExp 5, Plus, NumExp 3)), CompoundStm (AssignStm ("b", EseqExp (PrintStm [IdExp "a", OpExp (IdExp "a", Minus, NumExp 1)], OpExp (NumExp 10, Times, IdExp "a"))), PrintStm [IdExp "b"]))
...
fun maxargs stm =
  let
    ...
    fun searchExp exps =
      let
        fun f ([], stms) = stms
          ...
          | f (OpExp (left, _, _) :: exps, stms) = f (left :: left :: exps, stms)
    ...
    fun f ([], n) = n
      ...
      | f (AssignStm (_, exp) :: stms, n) = f (searchExp [exp] @ stms, n)
...
fun interprets stm =
  let
    ...
    fun opEval (Plus, a, b) = a + b
      ...
      | opEval (Div, a, b) = a div b
    (* TODO float point number *)
    fun interpStm (stm, table) =
      let
        fun interpExp (IdExp id, table) = (getOpt (lookup (table, id), 0), table)
          ...
          let
            (* TODO raise an exception instead of default to 0 *)
